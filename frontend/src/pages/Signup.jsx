...
export default function Signup() {
    ...
    const performSignup = async () => {
        setSubmitting(true);
        try {
            // passwordConfirm is client-side only; backend only needs the rest.
            ...
            if (err.status === 409) {
                setError('Could not create account. If you already have one, please log in.');
            } else if (err.status === 400) {
                // Surface the first field error from the validator if present.
                const firstField = err.errors && Object.keys(err.errors)[0];
                const msg = firstField ? err.errors[firstField][0] : 'Please check your input.';
                setError(msg);
    ...
    const handleSubmit = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) performSignup();
        });
    };
                ...
                <div className="p-10">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="relative pt-6">
                                <label className="absolute top-0 left-1 text-xs font-bold uppercase text-gray-500">
                                    Password
                                </label>
                                <input
                        ...
                        <button
                            type="submit"
                            disabled={submitting}
                            className="w-full bg-[#ED1B24] hover:bg-red-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-4 rounded-xl shadow-lg shadow-red-100 transition-all active:scale-[0.98] mt-4"
                        >
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-100 text-center space-y-3">
                        <p className="text-sm text-gray-500">
                            ...
                            <Link to="/login" className="ml-2 text-[#0078c4] font-bold hover:text-blue-800">
                                Login
                            </Link>
                        </p>
